...
Версия `1.0.8` использует асинхронную загрузку чанков, требует права на команды, телепортирует только на пригодную поверхность, не отменяет ожидание при повороте камеры и не повторяет прежнюю точку слишком близко.
