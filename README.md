...
Версия `1.0.4` использует асинхронную загрузку чанков, чтобы RTP не подвешивал главный поток сервера, добавляет задержку перед телепортацией, не отменяет ожидание при повороте камеры и всегда доводит телепорт до конца.
